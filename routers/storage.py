...
def get_commission_storage(user = Depends(get_auth_user)):
    if not user:
        return JSONResponse(status_code=403, content={"error": True, "message": "未登入系統，拒絕存取"})
    try:
...
@router.get("/api/storage/commission/download/{commission_id}")
async def get_commission_download(commission_id:int, user = Depends(get_auth_user)):
    if not user:
        return JSONResponse(status_code=403, content={"error": True, "message": "未登入系統，拒絕存取"})
    try:
        ...
        if data["commission"]["is_downloaded"] == 0:
            ...
            await notification.add_notification(user["id"], user["username"], [product["owner"]["id"]], 7, [product["id"]], None)
...
def get_commission_storage(commission_id:int, user = Depends(get_auth_user)):
    if not user:
        return JSONResponse(status_code=403, content={"error": True, "message": "未登入系統，拒絕存取"})
    try:
...
@router.get("/api/storage/{product_id}")
def get_storage(product_id: int | None = None, user = Depends(get_auth_user)):
    if not user:
        return JSONResponse(status_code=403, content={"error": True, "message": "未登入系統，拒絕存取"})
    try:
