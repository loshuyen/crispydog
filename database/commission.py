...
def update_commission(commission_id, is_accepted=None, is_paid=None, is_delivered=None, is_downloaded=None):
    try:
        ...
        if is_accepted:
            ...
            params = (datetime.now(ZoneInfo("Asia/Taipei")), is_accepted, commission_id)
        elif is_paid:
            statement = f"{statement_start} is_paid = %s {statement_end}"
            params = (datetime.now(ZoneInfo("Asia/Taipei")), is_paid, commission_id)
        elif is_delivered:
            statement = f"{statement_start} is_delivered = %s {statement_end}"
            params = (datetime.now(ZoneInfo("Asia/Taipei")), is_delivered, commission_id)
        else:
            statement = f"{statement_start} is_downloaded = %s {statement_end}"
            params = (datetime.now(ZoneInfo("Asia/Taipei")), is_downloaded, commission_id)
...
def update_file_url(commission_id, file_url):
    try:
        db = pool.get_connection()
        cursor = db.cursor()
        cursor.execute("UPDATE commission SET file_url = %s, updated_at = %s WHERE id = %s", (file_url, datetime.now(ZoneInfo("Asia/Taipei")), commission_id))
        db.commit()
    except Exception as e:
        raise e
    finally:
