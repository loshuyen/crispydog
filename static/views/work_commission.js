...
export function render_commission_work(commission) {
    ...
    uploaded_photo.src = commission.photo_url;

    const photo_confirm_btn = document.querySelector(".commission__photo-confirm-btn");
    const progress = get_progress(commission)
    if (progress >= 1) {
    ...
    if (commission.file_url) {
        ...
        img.src = commission.file_url;
        delieverd_file.style.display = "block";
        title.style.display = "block";
    }
}

export function render_commission_progress(commission) {
    const progress = get_progress(commission);
